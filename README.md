...
```
2️⃣ 创建 ```assets/css/custom-buttons.css```
```css
/* 返回顶部按钮样式 */
.back-to-top {
    position: fixed;
...
```
4️⃣ 修改 ```_layouts/default.html```，在文件中找到 ```</body>``` 标签前，添加以下代码：
```html
...
```
5️⃣ 更新 ```_config.yml```，确保您的 ```_config.yml``` 文件包含以下配置：
```yaml
include:
  - assets
  - assets/js
...
- ```buttons.js```: 包含返回顶部和搜索功能的 JavaScript 代码

这些文件是相互独立的，不会影响您现有的功能。
